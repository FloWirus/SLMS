...
- A second, independent "Resize cover art" checkbox + size (px)/DPI fields at the bottom of the window shrinks embedded cover art on tracks copied to a device during that sync, keeping the longer side at the given size (aspect ratio kept) and stamping the given DPI. Same rules as the conversion checkbox: only library → device, the library file's cover is never touched, and neither the checkbox nor the field values are remembered between sessions.
