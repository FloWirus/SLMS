...
- Two-way sync, run manually: library → device, or device → library. Never deletes files, and asks before overwriting a file that differs from the source (unless "Force re-sync" is checked, which overwrites without asking — see below). The sync progress dialog has a Cancel button: cancelling stops before the next track is touched, so tracks already copied stay as they are and nothing already in flight is left half-written. Before starting, it warns if the selected tracks look larger than the free space on the device (an estimate you can override — tracks already there won't be copied again, and conversion usually shrinks files).
...
- Resized cover art cache: `~/.local/share/SLMS/covers/` (`covers.db` plus the cached image files). Keyed by the source artwork's own hash, so it is shared across albums, repeat syncs and every sync target — resizing a given cover once covers every device it's later synced to — and stays valid if the library folder is moved or renamed. Nothing is written into the music library itself; earlier versions left a `music_db/` and a `[Covers]/` folder in there, which can be deleted by hand.
...
music_sync/
  ...
  cover_cache.py              persistent resized-cover cache (in the app's data directory, keyed by artwork hash)
...
- Synchronizacja dwukierunkowa uruchamiana ręcznie: biblioteka → nośnik lub nośnik → biblioteka. Nigdy nic nie usuwa i pyta przed nadpisaniem pliku różniącego się od źródła (chyba że zaznaczony jest „Wymuś resync”, który nadpisuje bez pytania — patrz niżej). Okno postępu synchronizacji ma przycisk Anuluj: przerwanie zatrzymuje kopiowanie przed kolejnym utworem, więc utwory już skopiowane zostają bez zmian, a nic będącego akurat w trakcie zapisu nie zostaje w połowie napisane. Przed startem pojawia się ostrzeżenie, jeśli wybrane utwory wyglądają na większe niż wolne miejsce na nośniku (to szacunek, który można zignorować — utwory już tam obecne nie będą kopiowane ponownie, a konwersja zwykle zmniejsza pliki).
...
- Cache przeskalowanych okładek: `~/.local/share/SLMS/covers/` (`covers.db` i pliki graficzne). Kluczowany hashem samej okładki źródłowej, więc jest współdzielony między albumami, kolejnymi synchronizacjami i wszystkimi nośnikami — przeskalowanie danej okładki raz obejmuje każde urządzenie, na które trafi później — i pozostaje ważny po przeniesieniu czy zmianie nazwy folderu z muzyką. W samej bibliotece muzycznej nic nie jest zapisywane; wcześniejsze wersje zostawiały tam foldery `music_db/` i `[Covers]/`, które można usunąć ręcznie.
...
music_sync/
  ...
  cover_cache.py              trwały cache przeskalowanych okładek (w katalogu danych aplikacji, kluczowany hashem okładki)
